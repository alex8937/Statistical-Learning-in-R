...
matpoints(jitter(age),I(wage>250)/10,pch="|",cex=.5)
```

```{r}
fit0=glm(I(wage>250) ~ 1, data=Wage, family=binomial)
fit1=glm(I(wage>250) ~ poly(age,1), data=Wage, family=binomial)
fit2=glm(I(wage>250) ~ poly(age,2), data=Wage, family=binomial)
fit3=glm(I(wage>250) ~ poly(age,3), data=Wage, family=binomial)
anova(fit0, fit1, fit2, fit3, test = 'Cp')
```
